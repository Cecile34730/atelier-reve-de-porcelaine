...
export async function POST(request: Request) {
  try {
    ...
    const brevoResponse = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        sender: { name: "Atelier Rêve de Porcelaine", email: "contact@reve-de-porcelaine.fr" }, // L'adresse validée dans Brevo
        to: [{ email: "contact@reve-de-porcelaine.fr", name: "Atelier reve de porcelaine" }], // Cécile reçoit le mail
        subject: subject,
